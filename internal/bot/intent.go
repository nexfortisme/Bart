...
func MessageIntendedForBartClassifier(message string, stores map[string]*classifier.MemoryStore) string {
	result, err := classifier.NewClassifier(classifier.NewLMStudioEmbedder(os.Getenv("LLM_BASE_URL"), os.Getenv("EMBEDDING_MODEL")), stores["message_intent"]).Classify(message)
	if err != nil {
		return ""
	}
...
func ToolIntentClassifier(message string, stores map[string]*classifier.MemoryStore) string {
	result, err := classifier.NewClassifier(classifier.NewLMStudioEmbedder(os.Getenv("LLM_BASE_URL"), os.Getenv("EMBEDDING_MODEL")), stores["tool_intent"]).Classify(message)
	if err != nil {
		return ""
	}
